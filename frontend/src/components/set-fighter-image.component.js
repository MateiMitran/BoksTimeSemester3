...
export default class SetFighterImageComponent extends Component {
    ...
    componentDidMount() {
        ...
        WeightClassService.addFighter(localStorage.getItem("weight_class"), id).then((response)=> {
    ...
    handleSetImage(e) {
        ...
        if (this.checkBtn.context._errors.length === 0) {
            FighterService.addPicture(this.state.fighterId, Image).then((response)=> {
                console.log(response.data);
                window.location.href="/fighter?id="+this.state.fighterId;
            });
        }
    }
